// Prism auth. JWT bearer + legacy cookie (pr_sid). Secret from config/app.secret
// — leakable via the report download traversal (V8), enabling the admin forge (V9).
...
function sidFromReq(req) { const m = (req.headers.cookie || '').match(/pr_sid=([^;]+)/); return m ? decodeURIComponent(m[1]) : null }
