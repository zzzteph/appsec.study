...
<template>
  ...
  <div class="grid" style="grid-template-columns:1fr 1.4fr;align-items:start">
    <div class="card">
      ...
      <div v-for="m in msgs" :key="m.id" class="row" style="padding:10px 0;border-bottom:1px solid var(--line);cursor:pointer" @click="read(m)">
        <div><b>{{ m.subject }}</b><div class="muted" style="font-size:12px">{{ m.created }}</div></div><span class="grow"></span><span v-if="m.is_system" class="badge premium">bank</span>
      </div>
    </div>
