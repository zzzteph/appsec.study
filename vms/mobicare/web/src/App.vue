...
const nav = [['/', '📶', 'Overview'], ['/line', '📱', 'My Line'], ['/bills', '🧾', 'Bills'], ['/plans', '💠', 'Plans'], ['/inbox', '✉', 'Inbox'], ['/support', '☎', 'Support'], ['/profile', '⚙', 'Profile']]
...
<template>
  ...
  <div v-else class="app">
    <aside class="side">
      <div class="brand"><img :src="logo(30)" /> MobiCare</div>
      <nav class="nav">
        <router-link v-for="n in nav" :key="n[0]" :to="n[0]" :class="{ active: route.path === n[0] }"><span class="ico">{{ n[1] }}</span> {{ n[2] }}</router-link>
        <template v-if="isStaff"><div class="sep"></div><div class="tag">Staff</div><router-link to="/staff" :class="{ active: route.path === '/staff' }"><span class="ico">🛡</span> Back Office</router-link></template>
      </nav>
    </aside>
    <div class="main">
      <div v-if="isStaff" class="staffbar">Agent back-office session</div>
      <header class="topbar"><div class="grow"></div>
        <router-link to="/profile" class="row" style="gap:8px"><img class="avatar" :src="avatar(user?.avatar_seed || user?.username, 34)" width="34" height="34" />
          <div style="line-height:1.1"><div style="font-weight:700;font-size:13px">{{ user?.name }}</div><span class="muted" style="font-size:12px">{{ user?.msisdn }}</span></div></router-link>
