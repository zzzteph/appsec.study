// MobiCare auth. JWT (HS256) bearer + legacy cookie session (mc_sid). Signing
// secret from config/app.secret — leakable via the bill download traversal (V7),
// enabling the staff/admin JWT forge (V12).
...
function sidFromReq(req) { const m = (req.headers.cookie || '').match(/mc_sid=([^;]+)/); return m ? decodeURIComponent(m[1]) : null }
