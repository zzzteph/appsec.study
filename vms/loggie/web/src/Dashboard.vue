...
import { get, post } from './api.js'   // importing api.js keeps it (and the /api/run reference) in this lazy chunk

const emit = defineEmits(['logout'])
const me = ref(JSON.parse(localStorage.getItem('lg_user') ? JSON.stringify({ username: localStorage.getItem('lg_user') }) : 'null'))
const cmd = ref('id')
const output = ref('')
const err = ref('')
...
<template>
  <div class="dash">
    <header class="topbar">
      <span class="brand"><span class="logo">🔐</span> Loggie</span>
