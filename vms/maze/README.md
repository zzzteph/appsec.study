...
**maze** — a **self-mutating** vulnerable app built to be a *maze for scanners*. On every startup a
...
```
docker build -t maze ./vms/maze
docker run --rm -p 8080:80 maze                 # random machine → http://localhost:8080
docker run --rm -p 8080:80 -e MAZE_SEED=demoA maze   # replay an EXACT machine
docker run --rm -p 8080:80 -e MAZE_API=graphql maze  # pin the transport (rest|graphql|traditional)
```

**Restart to re-mutate.** There's no runtime "mutate" button — the app regenerates itself each time the
container (re)starts (the fleet restarts every ~2h). The same `MAZE_SEED` always reproduces the same
...
#### Three transports (axis: `mut.api`, or pin with `MAZE_API`)
...
```
bash vms/maze/tools/verify.sh 40
```
