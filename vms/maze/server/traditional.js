...
<title>${esc(title)} · maze</title><style>
...
</style></head><body><header>◐ maze <span class="chip" style="background:rgba(255,255,255,.2);color:#fff">traditional · ${esc(mut.auth)}</span></header>
    ...
    try {
      ...
      if (op === 'login') {
        ...
        if (on('remember-me') && remember) {
          ...
          res.setHeader('Set-Cookie', [].concat(prev, 'maze_remember=' + rv + '; Path=/; HttpOnly'))
        }
