...
const MOUNT = {
  ...
  disclosure(x) {
    ...
    const spec = () => {
      const spec = { openapi: '3.0.0', info: { title: 'maze API', version: '1.0' }, paths: {} }
  ...
  account(x) {
    ...
    router.post(base + '/register', (q, s) => {
      ...
      db.prepare('INSERT INTO users(username,password,role,email,apikey) VALUES (?,?,?,?,?)').run(username, password, r, username + '@maze.local', 'ak_live_' + Math.random().toString(16).slice(2, 10))
    ...
    router.post(base + '/login', (q, s) => {
      ...
      if (on('remember-me') && remember) {
        ...
        s.setHeader('Set-Cookie', [].concat(prev, 'maze_remember=' + val + '; Path=/; HttpOnly'))
      }
...
function mountSide(x) {
  ...
  // ---- bugbounty-monitor (CWE) taxonomy side vulns ----
  if (on('side-weak-hash')) { router.get(base + '/pwhash', (q, s) => { const u = db.prepare('SELECT username,password FROM users WHERE username=?').get(q.query.user || 'alice'); if (!u) return s.status(404).json({ error: 'not found' }); const algo = variant('side-weak-hash') || 'md5-unsalted'; const h = require('crypto').createHash(algo === 'sha1' ? 'sha1' : 'md5').update(u.password).digest('hex'); s.json({ username: u.username, algo, hash: h }) }); add('GET', base + '/pwhash', 'pwhash') }
  if (on('side-no-ratelimit')) { router.get(base + '/auth-config', (q, s) => s.json({ rateLimit: false, lockout: false, mfa: false, captcha: false })); add('GET', base + '/auth-config', 'auth-config') }
  if (on('side-session-fixation')) { router.post(base + '/fixate', (q, s) => { const sid = (q.body && q.body.sid) || 'fixed'; s.setHeader('Set-Cookie', 'maze_sid=' + sid + '; Path=/'); s.json({ ok: true, accepted: sid }) }); add('POST', base + '/fixate', 'fixate') }
  if (on('side-unicode-collision')) { router.post(base + '/recover', (q, s) => { const raw = String((q.body && q.body.username) || ''); const norm = raw.normalize('NFKC').toLowerCase(); const exists = db.prepare('SELECT 1 FROM users WHERE lower(username)=?').get(norm); s.json({ normalized: norm, collides: !!exists }) }); add('POST', base + '/recover', 'recover') }
