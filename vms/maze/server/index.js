...
// Mutate ONCE at startup, deterministically from a seed (env MAZE_SEED replays an exact machine).
// Transport axis: MAZE_API env pins one (rest/graphql/traditional) — otherwise the engine picks
...
const apis = process.env.MAZE_API ? [process.env.MAZE_API] : undefined
const mut = generate({ apis, seed: process.env.MAZE_SEED || undefined })
const auth = makeAuth(mut)
console.log('maze gen ' + mut.generation + ' seed=' + mut.seed + ' api=' + mut.api + ' auth=' + mut.auth + ' | chain: ' + mut.solutionKind)
...
app.listen(80, () => console.log('maze on :80'))
