#!/usr/bin/env bash
# Full maze verification gate. Run from vms/maze:  bash tools/verify.sh [N]
...
docker run --rm maze node validate.js | grep -Ei "unsolvable|violation|non-reprod|distinct chain|re-valid"

echo "==> [3/3] boot + blind solve ($N fresh mutations)"
docker rm -f mu >/dev/null 2>&1 || true
docker run -d --name mu -p 8092:80 maze >/dev/null
