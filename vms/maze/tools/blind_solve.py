...
def cred_from_login(auth, resp_text, resp_headers):
    ...
    if auth == "session":
        sc = resp_headers.get("Set-Cookie", "");  m = re.search(r'maze_sid=([^;]+)', sc)
        if m: return {"Cookie": "maze_sid=" + m.group(1)}
    return None
...
def admin_creds(eps, auth, facts):
    ...
    if auth == "session":
        cands.append(("session-predict", {"Cookie": "maze_sid=s-1000"}))
        if facts.get("sid"): cands.append(("session-file", {"Cookie": "maze_sid=" + facts["sid"]}))
        # remember-me — either base64(username) or plain username
        cands.append(("remember-me-b64", {"Cookie": "maze_remember=" + base64.b64encode(ADM.encode()).decode()}))
...
def gql_cred_from_login(li, headers):
    ...
    sc = headers.get("Set-Cookie", "");  m = re.search(r'maze_sid=([^;]+)', sc)
    if m: return {"Cookie": "maze_sid=" + m.group(1)}
    return None
