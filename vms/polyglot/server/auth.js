// Polyglot auth. JWT bearer + legacy cookie (pg_sid). Secret from config/app.secret
// — leakable via the locale export traversal (V3), enabling the admin forge (V7).
...
function sidFromReq(req) { const m = (req.headers.cookie || '').match(/pg_sid=([^;]+)/); return m ? decodeURIComponent(m[1]) : null }
