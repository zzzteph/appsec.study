...
async function submit() {
  ...
  try {
    const d = await post(tab.value === 'login' ? '/idp/login' : '/idp/register', f.value)
    setUser(d.user); router.push(route.query.next || '/')
  ...
  <div class="auth">
    <div class="box">
      <div class="center" style="flex-direction:column;gap:8px;margin-bottom:22px">
        <img :src="logo(52)" width="52" height="52" />
        <h1 style="margin:0">Meridian ID</h1>
        <div class="muted">Single sign-on for your team</div>
      </div>
      <div class="card">
        <div class="tabs">
