...
<template>
  ...
  <div v-if="me" class="grid g2" style="align-items:start">
    <div class="card">
      <div class="row" style="margin-bottom:12px"><img class="avatar" :src="avatar(me.username, 52)" width="52" height="52" /><div><h3 style="margin:0">{{ me.name }}</h3><span class="muted">{{ me.email }}</span></div></div>
      <div class="field"><label>Display name</label><input v-model="form.name" /></div>
      <button class="btn primary" @click="save">Save</button>
      <div class="row" style="margin-top:14px"><span class="muted">Storage</span><b>{{ me.used_mb }} MB / {{ me.quota_gb }} GB</b></div>
    </div>
