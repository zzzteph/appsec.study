...
insU.run(ADMIN_USER, ADMIN_PASS, 'admin', 'root@mutie.local', 'Root Admin', 'Keeps the lights on.', 'app administrator', ADMIN_KEY)
insU.run('alice', 'sunflower12', 'user', 'alice@mutie.local', 'Alice Carter', C.pick(C.BIOS), '', 'ak_live_' + Math.random().toString(16).slice(2, 10))
insU.run('bob', 'qwerty2024', 'user', 'bob@mutie.local', 'Bob Nguyen', C.pick(C.BIOS), '', 'ak_live_' + Math.random().toString(16).slice(2, 10))
insU.run('carol', 'letmein!', 'partner', 'carol@mutie.local', 'Carol Patel', C.pick(C.BIOS), 'partner rep', 'ak_live_' + Math.random().toString(16).slice(2, 10))
const usernames = ['alice', 'bob', 'carol']
for (let i = 0; i < 26; i++) {
  const name = C.fullName(); const un = C.usernameFrom(name, i)
  usernames.push(un)
  insU.run(un, 'pw_' + Math.random().toString(16).slice(2, 8), C.chance(0.15) ? 'partner' : 'user', un + '@mutie.local', name, C.pick(C.BIOS), '', 'ak_live_' + Math.random().toString(16).slice(2, 10))
}
